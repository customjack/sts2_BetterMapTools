#!/usr/bin/env bash
# Shared .env loader for RoutingHelper bash scripts.

if [[ -n "${RH_ENV_LOADED:-}" ]]; then
  return 0
fi

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PROJECT_ROOT="$(cd "${SCRIPT_DIR}/../.." && pwd)"
ENV_FILE="${RH_ENV_FILE:-${PROJECT_ROOT}/.env}"
...
export RH_ENV_FILE="${ENV_FILE}"
export RH_ENV_LOADED=1
